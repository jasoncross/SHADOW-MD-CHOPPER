...
String PS3ControllerFootMac = "00:07:04:0A:33:85";  //Set this to your FOOT PS3 controller MAC address
String PS3ControllerDomeMAC = "00:07:04:BB:41:7B";  //Set to a secondary DOME PS3 controller MAC address (Optional)
...
String PS3ControllerBackupFootMac = "00:07:04:0E:D5:C8";  //Set to the MAC Address of your BACKUP FOOT controller (Optional)
String PS3ControllerBackupDomeMAC = "00:06:F7:4E:CE:B5";  //Set to the MAC Address of your BACKUP DOME controller (Optional)
...
byte domeAutoSpeed = 80;     // Speed used when dome automation is active - Valid Values: 50 - 100 
byte domeAutoMinTime = 1;   // Time between random dome moves
byte domeAutoMaxTime = 10;  // Time between random dome moves
int randomDomeAutoSpeed = 1; // 1 to enable.  Must still set value above
int auto360TurnOdds = 20;        // 1 in X chance of doing a 360 
...
byte domeAutoMinSpeed = 30;   //  min speed for auto dome
byte domeAutoMaxSpeed = 100;  // max speed for auto dome

int autoDomeSounds = 1;     // set to 1 to play sounds as dome makes auto movements
int domeWasMoving = 0;

int reedSwitch = A15;        // Arduino input for reed switch - set to 0 if not using
int reedStatus = 0;

// For random sounds, blended into banks for General (bank1), Chatty (bank2), and Responses (bank3)
// Set to 0 otherwise
int generalSoundCount = 7;
int chatSoundCount = 9;
int responseSoundCount = 15;
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnLeft_CIRCLE_cust_MP3_num = 194;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnRight_CIRCLE_cust_MP3_num = 195;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnLeft_PS_cust_MP3_num = 191;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnRight_PS_cust_MP3_num = 192;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnDown_PS_cust_MP3_num = 193;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnLeft_L1_cust_MP3_num = 187;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnRight_L1_cust_MP3_num = 188;  
...
// CUSTOM SOUND SETTING: Enter the file # prefix on the MP3 trigger card of the sound to play (0 = NO SOUND)
// Valid values: 0 or 182 - 200  
int btnDown_L1_cust_MP3_num = 189;  
...
int domeAutoTurnSpeed = domeAutoSpeed;
...
void setup()
{
    ...
    if (reedSwitch != 0) {
      pinMode(reedSwitch, INPUT); // start reed switch input
      Serial.print(F("\r\nUsing reed switch for auto dome"));
    }
...
{   
    ...
    if (domeAutomation && time360DomeTurn > 1999 && time360DomeTurn < 8001 && domeAutoSpeed > 0 && domeAutoSpeed < 101)  
    {
  ...
  if (type == 2) // Custom Button Configuration
  {
      ...
      if (MP3_num > 181 && MP3_num < 201) // Valid Custom Sound Range Selected - Play Custom Sound Selection
      {
        ...
        switch (MP3_num)
        {
          ...
          case 182:
          // up
            if (generalSoundCount > 0) {
              int randomGeneralSound;
              do {
                randomGeneralSound = random(1,generalSoundCount + 1);
              } while (randomGeneralSound == lastSoundPlayed);
              Serial1.print("$1");
              Serial1.print(randomGeneralSound);
              Serial1.print("\r");
              lastSoundPlayed = randomGeneralSound;
            } else {
              Serial1.print("$1\r");
            }
             break;
          ...
          //left
           if (chatSoundCount > 0) {
              int randomChatSound;
              do {
                randomChatSound = random(1,chatSoundCount + 1);
              } while (randomChatSound == lastSoundPlayed);
              Serial1.print("$2");
              Serial1.print(randomChatSound);
              Serial1.print("\r");
              lastSoundPlayed = randomChatSound;
            } else {
              Serial1.print("$2\r");
            }
             break;
          ...
          case 184:
          //right
            if (responseSoundCount > 0) {
              int randomResponseSound;
              do {
                randomResponseSound = random(1,responseSoundCount + 1);
              } while (randomResponseSound == lastSoundPlayed);
              Serial1.print("$3");
              Serial1.print(randomResponseSound);
              Serial1.print("\r");
              lastSoundPlayed = randomResponseSound;
            } else {
              Serial1.print("$3\r");
            }
             break;
          ...
          case 185:
          //down
             Serial1.print("$71\r"); // tada sound
             Serial1.print("&25,\"A00133|2\r"); // holos to leia effect for 2 seconds
             break;
          ...
          //PS+up
             Serial1.print("$61\r");
             Serial1.print("&25,\"A00716|3\r");
             break;
             
          case 191:
          //PS+left
             Serial1.print("$62\r");
             Serial1.print("&25,\"A00416|2\r");
             break;
             
          case 192:
          //PS+right
             Serial1.print("$63\r");
             Serial1.print("&25,\"A007|3\r");
             break;
          
          case 193:
          //PS+Down
             Serial1.print("$14\r");
             Serial1.print("&25,\"A00409|4\r");
             break;
             
          case 194:
          //circle+left
             Serial1.print("&25,\"A096\r");
             break;
             
          case 195:
          //circle+right
             Serial1.print("&25,\"A098\r");
             break;
...
void autoDome()
{
    ...
    if (domeStatus == 0)  // Dome is currently stopped - prepare for a future turn
    {
        ...
        if (randomDomeAutoSpeed == 1) {
          domeAutoTurnSpeed = random(domeAutoMinSpeed,domeAutoMaxSpeed+1);
        } else {
          domeAutoTurnSpeed = domeAutoSpeed;
        }
        
        if (domeTargetPosition == 0)  // Dome is currently in the home position - prepare to turn away
        {
          
            domeStartTurnTime = millis() + (random(domeAutoMinTime, domeAutoMaxTime) * 1000); // time between random moves

            leftRightOdds = random(2);
            if (leftRightOdds == 0) { //turn right
              rndNum = random(5,95);
            } else { // turn left
              rndNum = random(265,355);
            }

            randTurnOdds = random(auto360TurnOdds);
            output += randTurnOdds;
          output += "\r\n"; 
            if (randTurnOdds == auto360TurnOdds - 1) { // chance of a 360
              domeTargetPosition = 360;
            } else {
            ...
            if (domeTargetPosition == 360) { // spin the dome
              domeAutoTurnSpeed = domeAutoSpeed;
              if (random(2) == 1) { // odds of which direction to spin
                domeTurnDirection = 1;
              } else {
                domeTurnDirection = -1;
              }
            ...
            else if (domeTargetPosition < 180)  // Turn the dome in the positive direction
            {
        ...
        } else  // Dome is not in the home position - send it back to home
        {
            ...
            if (domeTargetPosition < 180 || (domeTargetPosition == 360 && domeTurnDirection == 1))
            {
        ...
        domeStatus = 1;  // Set dome status to preparing for a future turn
          ...
          output += "\r\n";       
          output += "Dome Turn Speed: ";
          output += domeAutoTurnSpeed;
          output += "\r\n";     
        #endif
    ...
    {
        if (domeWasMoving == 0) {
          // Do things while dome starts to move
          int randomSoundType;
          randomSoundType = random(1,4);
          if (randomSoundType == 1) {
            if (generalSoundCount > 0) {
              int randomGeneralSound;
              do {
                randomGeneralSound = random(1,generalSoundCount + 1);
        ...
        domeSpeed = domeAutoTurnSpeed * domeTurnDirection;
        if (reedSwitch != 0)  // target position only matters if using a reed
        {
          if (domeTargetPosition == 0) { // Dome is actively turning to head home
            
            if (digitalRead(reedSwitch) != 0) // reedStatus becomes zero when magnet is detected so turn until then
            {
                  if (domeStopTurnTime - millis() < 250) {
                     domeSpeed = domeSpeed * 0.5;
                  }
                  SyR->motor(domeSpeed);
                 
                 #ifdef SHADOW_DEBUG
                    output += domeSpeed;
                    output += ":Looking for Home Now!!\r\n";
                 #endif      
            } else  // turn completed - stop the motor
            {
                  ...
                  #ifdef SHADOW_DEBUG
                     output += "HOME FOUND - STOP TURN!!\r\n";
                  #endif
            }
          } else { // Dome is actively turning away from home
